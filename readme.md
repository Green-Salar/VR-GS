Hello, Developing a VR platform optimizing for Quest 3, on top of anothed SDK which he did great job on the gpu side to integrate the guassian splats code fro mthe paper to Unity. Hereunder you can some information about how to set up the scene. 
Check the tools ro import your new GS and be sure about placing them.
...
:warning: Status as of 2023 December: The owner is not planning any significant further developments on the SDK but not on this platform any update would be on the list to integrate.

:warning: The only platforms where this is known to work are the ones that use D3D12, Metal or Vulkan graphics APIs. Right now we are testing on the Quest 3, quest 2 and the Meta realted GPU configs would work too. Also
PC (Windows on D3D12 or Vulkan), Mac (Metal), Linux (Vulkan) should work. Anything else the owner have not actually tested;
it might work or it might not.
- this project is setup and optimised for Quest 3 right now
- Some virtual reality devices work (reportedly HTC Vive, Varjo Aero, Quest 3 and Quest Pro). Some others might not
  work, e.g. Apple Vision Pro. 
- Anything using OpenGL or OpenGL
- 
## Usage

Right now it set up fully setup, jump inside and check.

On the base if you start there  check their repo.
...
The base SDK blog posts about all this:
...
----- for QUest 3 it need to be tested
